...
std::vector<SOCKET_INFO>
sck_list; // 연결된 클라이언트 소켓들을 저장할 배열 선언.
SOCKET_INFO server_sock; // 서버 소켓에 대한 정보를 저장할 변수 선언.
int client_count = 0; // 현재 접속해 있는 클라이언트를 count 할 변수 선언.
void server_init(); // socket 초기화 함수. socket(), bind(), listen() 함수
// 실행됨. 자세한 내용은 함수 구현부에서 확인.
void add_client(
    int a); // 소켓에 연결을 시도하는 client를 추가(accept)하는 함수. client
// accept() 함수 실행됨. 자세한 내용은 함수 구현부에서 확인.
void send_msg(
    const char* msg,
    string sender); // send() 함수 실행됨. 자세한 내용은 함수 구현부에서 확인.
void recv_msg(string user_name); // recv() 함수 실행됨. 자세한 내용은 함수
// 구현부에서 확인.
void del_client(
    int idx); // 소켓에 연결되어 있는 client를 제거하는 함수. closesocket()
// 실행됨. 자세한 내용은 함수 구현부에서 확인.
...
string log_in(string id, string pw);
...
SOCKET_INFO get_socket_info(string user) {
    for (int i = 0; i < sck_list.size();
        i++) { // 접속해 있는 모든 client에게 메시지 전송
...
void server_init() {
    ...
    server_addr.sin_addr.s_addr =
        htonl(INADDR_ANY); // 서버이기 때문에 local 설정한다.
    // Any인 경우는 호스트를 127.0.0.1로 잡아도 되고 localhost로 잡아도 되고 양쪽
    // 다 허용하게 할 수 있따. 그것이 INADDR_ANY이다. ip 주소를 저장할 땐
    // server_addr.sin_addr.s_addr -- 정해진 모양?

    bind(server_sock.sck, (sockaddr*)&server_addr,
        sizeof(server_addr)); // 설정된 소켓 정보를 소켓에 바인딩한다.
...
void add_client(int ti) {
    ...
    // Winsock2의 recv 함수. 'client'가 보낸 닉네임을 받음.
    ...
    if (msg == "sign_in") {
        ...
        while (flag) {
            if (recv(new_client.sck, buf, MAX_SIZE, 0) > 0) {
                string answer = check_id(buf);
                send(new_client.sck, answer.c_str(), MAX_SIZE, 0);
                if (answer == "Y") {
                    flag = false;
                    break;
                }
        ...
        recv(new_client.sck, buf, MAX_SIZE, 0);
        msg = buf;
        std::stringstream buf2(msg);
        buf2 >> name >> id >> pw >> birth;
        pstmt = con->prepareStatement(
            "INSERT INTO userinfo(name, id, pw, birthday) VALUES(?,?,?,?)");
        ...
        if (recv(new_client.sck, buf, MAX_SIZE, 0) > 0) {
            ...
            if (msg == "change_info") {
                ...
                flag = true;
                string name, id, pw, birth, beforeID;
                if (recv(new_client.sck, buf, MAX_SIZE, 0) > 0) {
                    msg = buf;
                    std::stringstream buf2(msg);
                    buf2 >> name >> id >> pw >> birth >> beforeID;
                    pstmt = con->prepareStatement("UPDATE userinfo SET name = ?, id = ?, "
                        "pw = ?, birthday = ? WHERE id = ?");
                    pstmt->setString(1, name);
                    pstmt->setString(2, id);
                    pstmt->setString(3, pw);
                    pstmt->setString(4, birth);
                    ...
                    try {
                        pstmt->execute(); // SQL 쿼리 실행
                        cout << "User " << name << " updated to the database.";
                    }
                    catch (sql::SQLException& e) {
                        cout << "SQL Error: " << e.what();
                    }
                ...
                if (msg.find("*") != string::npos) {
                    ...
                    new_client.user = msg;
                    string msg =
                        "[공지] " + new_client.user + " 님이 성인채팅방에 입장했습니다.";
                    cout << msg << endl;
                ...
                if (msg.find("#") != string::npos) {
                    ...
                    new_client.user = msg;
                    string msg =
                        "[공지] " + new_client.user + " 님이 게임방에 입장했습니다.";
                    cout << msg << endl;
                    flag = false;
                }
                ...
                if (!new_client.hasSentEnterMsg && flag) {
                    string msg =
                        "[공지] " + new_client.user + " 님이 전체채팅방에 입장했습니다.";
                    cout << msg << endl;
                ...
                // 다른 사람들로부터 오는 메시지를 계속해서 받을 수 있는 상태로 만들어
                // 두기.

                client_count++; // client 수 증가.

                cout << "[공지] 현재 접속자 수 : " << sck_list.size() << "명" << endl;
                // send_msg(msg.c_str(), new_client.user); // c_str : string 타입을
                // const chqr* 타입으로 바꿔줌.
...
void send_msg(const char* msg, string sender) {
    ...
    int count = 0;
    for (int i = 0; i < sck_list.size();
        i++) { // sender 제외한 모든 client에게 메시지 전송
...
void if_badwords(int ti, const char* msg) {
    ...
    cout << "욕설 감지!" << endl;
    text = "[경고] " + nickname +
        "님이 욕설을 사용하셨습니다! 누적 경고 횟수 : " +
        std::to_string(sck_list[ti].count);
    send_msg(text.c_str(), "");
    if (sck_list[ti].count == 3) {
        sck_list[ti].count = 0;
        text =
            "[공지] " + nickname + "님이 경고누적으로 3분간 대화하실 수 없습니다!";
        send_msg(text.c_str(), "");
        ...
        while (t > 0) {
            ...
            if (bytes_received >
                0) { // 오류가 발생하지 않으면 recv는 수신된 바이트 수를 반환. 0보다
                // 크다는 것은 메시지가 왔다는 것.
                text = "[공지] 대화에 참여하실 수 없습니다(남은 시간 : " +
                    std::to_string(min) + "분" + std::to_string(sec) + "초)";
...
string check_id(string id) {
    ...
    if (result->next()) {
        ...
        if (db_id == id) {
            answer = "N";
            send(server_sock.sck, answer.c_str(), 1, 0);
        }
    }
    else
        answer = "Y";
...
string log_in(string id, string pw) {
    string answer = "N";
    pstmt = con->prepareStatement(
        "SELECT id, pw FROM userinfo WHERE id = ? AND pw = ?");
    pstmt->setString(1, id);
...
void recv_msg(string user) {
    ...
    while (1) {
        ...
        if (recv(sck_info.sck, buf, MAX_SIZE, 0) >
            0) { // 오류가 발생하지 않으면 recv는 수신된 바이트 수를 반환. 0보다
            // 크다는 것은 메시지가 왔다는 것.
            nickname = user;
            msg = buf;
            ...
            result = stmt->executeQuery(
                "SELECT id, score FROM game_rank order by score desc limit 1");
            if (result->next()) {
            ...
            if (first == user) {
                if (first == game_id || game_id == "") {
                    ...
                    msg = "[게임공지] : " + first +
                        "가 1등입니다! 점수 : " + std::to_string(game_score);
                }
                else {
                    if (game_score > stoi(id))
                        msg = "[게임공지] : " + game_id +
                        "가 여전히 1등입니다! 점수 : " + std::to_string(game_score);
                    else
                        msg = "[게임공지] :  " + first + "가 " + game_id +
                        "을 이겼습니다! 점수 : " + id;
                }
                cout << msg << endl;
            ...
            if (msg == "/out_adult") {
                num = 1;
                int adult_idx =
                    find(chattingroom_member.begin(), chattingroom_member.end(),
                        nickname) -
                    chattingroom_member.begin(); //성인채팅방을 나가는 사람의 idx찾기
            ...
            for (int i = 0; i < sck_list.size(); i++) {
                if (id == sck_list[i].user && first == "/w") {
                    ...
                    pstmt =
                        con->prepareStatement("INSERT INTO private_msg(sender, receiver "
                            ",message, time) VALUES(?,?,?,?)");
            ...
            if (msg.find("*") != string::npos && num == 0) {
                ...
                pstmt = con->prepareStatement(
                    "INSERT INTO chat(nickname,message, time) VALUES(?,?,?)");
...
void del_client(int idx) {
    ...
    sck_list.erase(sck_list.begin() +
        idx); // 배열에서 클라이언트를 삭제하게 될 경우 index가
    // 달라지면서 런타임 오류 발생....ㅎ
    client_count--;
}
int removeSocket(string user) {
...
int main() {
    WSADATA wsa;
    start_sql();
    // Winsock를 초기화하는 함수. MAKEWORD(2, 2)는 Winsock의 2.2 버전을
    // 사용하겠다는 의미. 실행에 성공하면 0을, 실패하면 그 이외의 값을 반환. 0을
    // 반환했다는 것은 Winsock을 사용할 준비가 되었다는 의미.
    ...
    if (!code) {
        ...
        for (int i = 0; i < MAX_CLIENT; i++) {
            // 인원 수 만큼 thread 생성해서 각각의 클라이언트가 동시에 소통할 수
            // 있도록 함.
        ...
        while (
            1) { // 무한 반복문을 사용하여 서버가 계속해서 채팅 보낼 수 있는 상태를
            // 만들어 줌. 반복문을 사용하지 않으면 한 번만 보낼 수 있음.
            string text, msg = "";
